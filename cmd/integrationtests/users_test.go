...
func testUserLifecycle(t *testing.T, cl *client.Client) {
	...
	resp, err := cl.LoginUser(ctx, &client.LoginUserRequest{
		Email:    "integration@test.com",
		Password: "pass",
	})
	require.NoError(t, err)
	...
	getUserResp, err := cl.GetUser(ctx, &client.GetUserRequest{Token: resp.Token})
	require.NoError(t, err)
	...
	err = cl.LogoutUser(ctx, &client.LogoutUserRequest{Token: resp.Token})
	require.NoError(t, err)
